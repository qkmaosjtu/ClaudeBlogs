...
- 分类：Claude Code
- 产品：Claude Code
- 关键词：agent, agents, Claude Code, skills, hooks, subagents, API, context, security, workflow

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、企业治理与安全、模型能力与上下文。官方摘要指出：When to delegate research, parallelize tasks, or get a fresh review with Claude Code subagents—and when to stick with the main session. 从正文结构看，文章围绕 What is a subagent?、When should you use subagents?、Research-heavy tasks、Multiple independent tasks 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：What is a subagent?
- 正文重点章节：When should you use subagents?
- 正文重点章节：Research-heavy tasks
- 正文重点章节：Multiple independent tasks
