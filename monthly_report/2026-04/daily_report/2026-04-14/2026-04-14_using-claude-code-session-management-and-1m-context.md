...
- 分类：Claude Code
- 产品：Claude Code
- 关键词：agent, agents, Claude Code, subagents, context, 1M

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、知识工作与办公生产力、模型能力与上下文。官方摘要指出：Learn how to manage context in Claude Code—when to continue, rewind, compact, or clear a session, and how subagents keep parent context clean. 从正文结构看，文章围绕 A quick primer on context, compaction and context rot、Every turn as a branching point、When to start a new session、Rewinding instead of correcting 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：A quick primer on context, compaction and context rot
- 正文重点章节：Every turn as a branching point
- 正文重点章节：When to start a new session
- 正文重点章节：Rewinding instead of correcting
