...
- 分类：Agents
- 产品：Claude Platform
- 关键词：agent, agents, Claude Code, Managed Agents, MCP, skills, hooks, subagents, API, context

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、企业治理与安全、知识工作与办公生产力、模型能力与上下文。官方摘要指出：Patterns for building effective MCP integrations: server design, OAuth with CIMD and vaults, context-efficient clients, and skills. Plus where MCP fits alongside direct API calls and CLIs for connecting agents to your systems. 从正文结构看，文章围绕 Connecting agents to external systems、Direct API calls、Command-line interface (CLI)、Model Context Protocol (MCP) 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：Connecting agents to external systems
- 正文重点章节：Direct API calls
- 正文重点章节：Command-line interface (CLI)
- 正文重点章节：Model Context Protocol (MCP)
