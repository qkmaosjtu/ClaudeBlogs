...
- 分类：Agents
- 产品：Claude Platform
- 关键词：agent, agents, Claude Code, skills, subagents, context, security, compliance, workflow, data

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、企业治理与安全、知识工作与办公生产力、模型能力与上下文。官方摘要指出：Five multi-agent coordination patterns, their trade-offs, and when to evolve from one to another. 从正文结构看，文章围绕 Pattern 1: Generator-verifier、How it works、Where it works well、Where it struggles 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：Pattern 1: Generator-verifier
- 正文重点章节：How it works
- 正文重点章节：Where it works well
- 正文重点章节：Where it struggles
