...
- 分类：Claude Code
- 产品：Claude Code
- 关键词：agent, agents, Claude Code, MCP, security

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、企业治理与安全、知识工作与办公生产力。官方摘要指出：In Claude Code, Claude writes, tests, and debugs code alongside you, navigating your codebase, editing multiple files, and running commands to verify its work. 从正文结构看，文章围绕 Our current approach to keeping users secure、Sandboxing: a safer and more autonomous approach、Two new sandboxing features in Claude Code、Sandboxed bash tool: safe bash execution without permission prompts 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- In Claude Code, Claude writes, tests, and debugs code alongside you, navigating your codebase, editing multiple files, and running commands to verify its work.
- 正文重点章节：Our current approach to keeping users secure
- 正文重点章节：Sandboxing: a safer and more autonomous approach
- 正文重点章节：Two new sandboxing features in Claude Code
- 正文重点章节：Sandboxed bash tool: safe bash execution without permission prompts
