...
- 产品：Claude Platform
- 关键词：agent, agents, Managed Agents, hooks, subagents, API, context, workflow, legal

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、知识工作与办公生产力、行业落地、模型能力与上下文。官方摘要指出：Dreaming, outcomes, and multiagent orchestration are now available in Claude Managed Agents. Build agents that learn, meet a quality bar, and work in parallel. 从正文结构看，文章围绕 Build self-improving agents with dreaming、Deliver better outcomes、Handle complex tasks with multiple agents、What teams are building 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：Build self-improving agents with dreaming
- 正文重点章节：Deliver better outcomes
- 正文重点章节：Handle complex tasks with multiple agents
