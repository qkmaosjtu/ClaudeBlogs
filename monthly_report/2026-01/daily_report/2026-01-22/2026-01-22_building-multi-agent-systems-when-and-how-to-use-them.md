...
- 分类：Agents
- 产品：Claude Platform
- 关键词：agent, agents, subagents, API, context, compliance, workflow, data, legal, marketing

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、企业治理与安全、知识工作与办公生产力、行业落地、模型能力与上下文。官方摘要指出：Most teams don&#39;t need multi-agent systems. Learn the three scenarios where they consistently outperform single agents—and how to implement them effectively. 从正文结构看，文章围绕 The case for starting with a single agent、A decision framework for multi-agent systems、Context protection、Parallelization 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：The case for starting with a single agent
- 正文重点章节：A decision framework for multi-agent systems
- 正文重点章节：Context protection
- 正文重点章节：Parallelization
