...
- 原文标题：Token-saving updates on the Anthropic API
- 原文链接：https://claude.com/blog/token-saving-updates
- 发布日期：2025-03-12
- 检测/生成时间：2026-06-13T12:17:15.239Z
- 分类：Product announcements
- 产品：Claude Platform
- 关键词：API, context, workflow, data, prompt caching

## 整体摘要

这篇文章聚焦于 智能体与工作流自动化、Claude Code 与软件工程、平台 API 与集成能力、知识工作与办公生产力、行业落地、模型能力与上下文。官方摘要指出：Claude now offers cache-aware rate limits, simplified prompt caching, and token-efficient tool use to help developers increase throughput and cut costs. We&#39;ve made several updates to the Anthropic API that let developers significantly increase throughput and reduce token usage with Claude 3. 从正文结构看，文章围绕 Increase your throughput with prompt caching、Cache-aware rate limits、Simpler cache management、Token-efficient tool use 展开，重点不只是功能发布，而是说明这些能力如何嵌入真实工作流、开发流程或企业治理框架。
...
- 正文重点章节：Increase your throughput with prompt caching
- 正文重点章节：Cache-aware rate limits
- 正文重点章节：Simpler cache management
- 正文重点章节：Token-efficient tool use
